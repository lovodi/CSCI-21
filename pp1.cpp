 ...
 * Using all the basics of C++
 *
 * Lovodi
 * Date created: 2/20/14
...
void countCharacters (string theString, int& alpha, int& num)
{
    int stringSize = theString.size();
    alpha = 0;
    num = 0;
    for (int i = 0; i < stringSize; i++)
    {
        if(isalpha(theString[i]))   //checks for alphabeticals
            alpha = alpha++;
        if(isdigit(theString[i]))   //checks for digits
            num = num++;
    }
}
...
string upAndDown(string theString)
{
    ...
    for (int i = 0; i < stringSize; i++)
    {
        ...
        {
            if((i % 2) == 0)
                output << static_cast<char>(toupper(theString[i])); //allows me to check characters in strings
            else
                output << static_cast<char>(tolower(theString[i]));
        }
    }
    return output.str();
}
...
int computeAverage (int values [], int arraySize)
{
    ...
    return total/ arraySize;    //dividing the added total with the number of ints in the array
}

int findMinValue (int values [], int arraySize)
{
    int minValue = values[0];   // sets the min value to the first number in the array
    for(int i = 0; i < arraySize; i++)
    {
        if(minValue > values[i])    //if the next number in the array is smaller, it'll chance
            minValue = values[i];   // the current min value with the smaler number.
    }
    return minValue;
}
int findMaxValue (int values [], int arraySize)
{
    int maxValue = values[0];   // sets the max value to the first value in the array
    for(int i = 0; i < arraySize; i++)
    {

/*
 * PC9
 * using classes and accessing private data
 *
 * Lovodi
 * Date created: 2/24/14
 * Last date modified: 2/24/14
 *
 * SOURCES USED
 *Rick Arriola
 */
...
{
    stringstream specifications;
    specifications.setf(ios::fixed);
    specifications << getName() << ", top speed: warp " << std::setprecision(2) << getTopSpeed() << ", fuel source: " <<
    getFuelSource() << ", crew capacity: " << getCrewCapacity();
    
    return specifications.str();
}

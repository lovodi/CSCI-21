...
string* makeDynoString (string contents)
{
    string* holder = new string(contents);
    return holder;
}
...
bool findWord (string* theString, string theWord)
{
    ...
    {
    	size_t const foundAt = theString->find(theWord);
        return foundAt != string::npos;
    }
    return true;
}
...
bool replaceWord (string* theString, string oldWord, string newWord)
{
    ...
    {
        size_t const foundAt = theString->find(oldWord);
        ...
        {
            theString->replace(foundAt, oldWord.length(), newWord, 0, newWord.length());
            return true;
        }
    }
    return true;
